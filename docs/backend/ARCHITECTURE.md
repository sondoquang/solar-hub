...
| `mailer` | Cấu hình SMTP (`MailSettings`, singleton `pk=1`, app-password mã hóa Fernet) + gửi email đơn hàng. `GET/PUT/PATCH /api/mail-settings/` (password write-only, không bao giờ trả về; `has_password` cho UI), `POST /api/mail-settings/test/` (gửi email thử). Digest tự động 9h/16h (Celery Beat, **chỉ đơn `genuine`**) + gửi thủ công đơn đã chọn (`POST /api/orders/send_email/`). Body HTML giống chi tiết đơn (template `mailer/orders_email.html`, tái dùng helper format của `orders.pdf`) + PDF đính kèm (mỗi trang một đơn, tái dùng `build_orders_pdf`). |
...
2. **Nhận theo tên (adoption) cho master CHƯA có mapping** — `_adopt_by_name` chạy **sau `_ensure_site_categories`, trước `_plan_site_push`**, **chỉ với site WooCommerce** (v1: Sapo dùng chung DB + mô hình field khác → bỏ qua adoption, push tới Sapo giữ cơ chế SKU/mapping cũ; tránh thêm 1 call `list_products`/store). Vì các site Woo chưa thống nhất SKU/id, một sản phẩm import về Hub có thể đã tồn tại sẵn trên site dưới tên gõ tay. `_adopt_by_name` liệt kê sản phẩm site (`WooClient.list_products`), index theo tên chuẩn hóa (`normalize_match_name` — trim+collapse+lowercase+**bỏ dấu tiếng Việt**, tắt được bằng `PRODUCT_MATCH_FOLD_DIACRITICS`), khớp `match_name` (đóng băng lúc import; rỗng → fallback `name`): **khớp đúng 1** → tạo `ProductMapping` (master rơi vào nhánh `update`, không tạo trùng); **khớp >1** (ambiguous) → KHÔNG nhận & KHÔNG tạo (loại khỏi run, ghi `SyncLog.detail["ambiguous"]`, run đánh PARTIAL); **0** → để bước sau create. Khớp tên chỉ chạy **một lần/(master,site)** và được ghi bền vào **`ProductAdoptionScan` (unique `(master, site)`)** cho **mọi** master đã quét (adopted / ambiguous / no-match), nên push sau **chỉ gọi `list_products` khi có master CHƯA từng quét** trên site đó — steady-state (catalog >1000 sp) không còn đọc lại toàn bộ catalog site mỗi lần. Master ambiguous lưu cờ `ambiguous=True` để các push sau **vẫn loại khỏi create** (không tạo trùng) mà không cần list lại, tới khi người dùng gỡ trùng và **`clear_product_sync_data --products`** xóa scan để quét lại. Đổi tên trên Hub về sau không phá liên kết (đã có mapping thì không quét). Số nhận ghi `SyncLog.detail["adopted"]`/`["adopted_count"]`.
...
6. **Đếm thành công per-item = có `id` VÀ không có `error`** — batch của Woo trả HTTP 200 kể cả khi item bị từ chối, và reject của update/delete **vẫn echo `id`** (vd `woocommerce_rest_product_invalid_id`), nên "có id" không đủ. **Tự lành mapping mồ côi:** update bị reject `invalid_id` (sản phẩm đã bị xóa trên site ngoài Hub, vd wp-admin) → gỡ `ProductMapping` (+ var-mapping) và **re-push như create ngay trong cùng run**; số lượng ghi `SyncLog.detail["recreated_stale"]`. **Tự lành ảnh không tải được:** Woo từ chối **CẢ bản ghi** với `woocommerce_product_image_upload_error` khi không sideload được ảnh sản phẩm (URL không truy cập được từ site — thường gặp khi `MasterProduct.images` còn hotlink từ site nguồn) → push **retry lại item đó BỎ trường `images`** trong cùng run, nên giá/mô tả/tồn kho **vẫn lên** và ảnh cũ trên site được giữ nguyên (update bỏ `images` = Woo không đổi gallery); SKU ghi vào `SyncLog.detail["image_skipped"]` và run đánh **PARTIAL** để nhắc re-host ảnh lên Hub.

**Ảnh trong mô tả (sideload — Woo only):** ảnh **gallery** (`MasterProduct.images`) luôn được Woo tự sideload qua trường `images:[{src}]` → thành media của site. Nhưng ảnh nhúng trong **HTML mô tả** (`<img src>`) mặc định chỉ **hot-link** về Hub. Khi `PRODUCT_SIDELOAD_DESCRIPTION_IMAGES=True` (mặc định), `_sideload_description_images` chạy **sau** batch chính (mọi master đã có mapping) cho ảnh Hub (`/media/...`) trong mô tả CHƯA có trên site: **Round 1** — 1 batch `update` gửi gallery hiện có (theo `id`, giữ nguyên) + ảnh mô tả mới (theo `src`, để Woo tải về); URL site Woo trả về được cache vào `catalog.SiteMediaAsset` (UNIQUE `(site, source_path)` — key theo path `/media/...` host-independent) và nạp vào `media_map`. **Round 2** — 1 batch `update` trả gallery về sạch (chỉ `id` cũ; ảnh vừa tải vẫn trong media library nhưng rời gallery) + mô tả đã rewrite `src` sang URL site. Ảnh **đã cache** thì `build_product_payload` rewrite ngay trong batch chính (0 call thừa); id gallery hiện có lấy từ response batch lần này (không có → `WooClient.get_product`). Mỗi ảnh sideload **1 lần/site** (re-push steady-state không tốn call). Lỗi mạng được nuốt, ghi `SyncLog.detail["description_images"]` (`{uploaded, products, skipped, error}`), `error` → run PARTIAL. **Sapo bỏ qua** (chưa có semantics sideload batch — follow-up).
...
**Nhập sản phẩm từ website chính (`import_products_from_site`):** lấy data gốc một chiều site → Hub (mirror `pull_categories_for_site`, fan-out 1 site qua `import_products_task` + lock `lock:import_products:{id}`, endpoint `POST /products/import_from_site/`, operation `import_products`). **v1 chỉ cho nhập từ site WooCommerce** — endpoint trả 400 nếu chọn site Sapo (Sapo có mô hình field khác, làm sau). Với mỗi sản phẩm **simple**: SKU trùng master sống → **link** (chỉ tạo mapping); chưa có → **create** master (đóng băng `match_name` = tên lúc nhập đã chuẩn hóa, `source_site`/`imported_at`); đã có mapping `(site, woo_id)` → skip (idempotent). **v1 chỉ nhập `simple`** — `variable/grouped/external` ghi vào `SyncLog.detail["skipped_types"]` (đẩy một master nửa-vời các loại này dễ làm hỏng sản phẩm trên site). SKU rỗng → placeholder `IMPORT-{site}-{woo_id}`. **Nội bộ hóa ảnh (`PRODUCT_INTERNALIZE_IMPORTED_IMAGES`, mặc định bật):** master MỚI tạo từ import được `_internalize_imported_media` **tải ảnh gallery + ảnh nhúng trong HTML mô tả** về media library Hub (`ProductImage`) rồi rewrite tham chiếu sang URL Hub (tuyệt đối qua `MEDIA_PUBLIC_BASE_URL`) — mirror đúng dạng sản phẩm tạo trên Hub. Nếu KHÔNG làm vậy, master giữ URL ảnh của **site nguồn**, và push sang **site khác** sẽ sideload thất bại (site đích thường không reach được site nguồn) → ảnh bị rớt (xem self-heal `image_skipped` ở §batch). Tải ảnh chạy **ngoài transaction** (I/O mạng không giữ transaction) và **song song** (`_prefetch_import_images` gom mọi URL ảnh remote của các master MỚI, tải đồng thời qua `ThreadPoolExecutor(PRODUCT_IMPORT_IMAGE_WORKERS`, mặc định 4) — chỉ fetch bytes trong thread, ghi `ProductImage` tuần tự trên main thread để ORM đơn luồng), rồi vòng per-item chỉ tra cache đã hâm nóng; dedup theo URL trong cùng run (kể cả ảnh chung giữa nhiều sản phẩm); link vào master sẵn có (SKU trùng) **không tải** (dùng lại ảnh Hub của master đó). Tải fail → giữ nguyên URL gốc (vẫn hot-link), đếm vào `SyncLog.detail["image_download_failures"]`; thành công đếm `["images_internalized"]`. webp được tái mã hóa PNG như đường upload (nhiều site WP từ chối sideload webp). **Vá ngược:** sản phẩm import TRƯỚC khi có cơ chế này vẫn giữ URL site nguồn (re-import idempotent bỏ qua woo_id đã map → không tự sửa); command `python manage.py internalize_imported_images [--site <id>] [--dry-run]` quét master đã import (`source_site` set) còn ref ảnh chưa phải Hub và nội bộ hóa bằng đúng helper trên (yêu cầu `MEDIA_PUBLIC_BASE_URL`, nếu không URL nội bộ hóa sẽ là `/media/...` tương đối, site không tải được và `set_media_public_url` cũng không vá được).
...
### 5.4. Gửi email đơn hàng (`apps/mailer`)

Hub gửi báo cáo đơn hàng qua email theo **2 đường**, cùng dựng một message: body HTML (`render.py` → template `mailer/orders_email.html`, bố cục giống chi tiết đơn — header đơn, khối khách hàng, bảng line-items + tổng) **kèm PDF** (`orders.pdf.build_orders_pdf`, mỗi trang một đơn). Helper format (`_money`/`_format_dt`/`_title_case_name`/`STATUS_LABELS`) tái dùng từ `orders.pdf` để email, PDF và UI khớp nhau.

- **Digest tự động (lịch cấu hình được từ UI):** Celery Beat chạy `dispatch_due_digests` **mỗi phút**; task gọi `services.run_scheduled_digest` để xét các mốc giờ trong `MailSettings.digest_times` (list `"HH:MM"`, giờ local `Asia/Ho_Chi_Minh`, mặc định `["09:00","16:00"]`). Khi mốc gần nhất trong ngày đã tới (`slot <= now`) và watermark `last_digest_sent_at` còn trước mốc đó → fire `send_digest`. So sánh `<=` nên **tự bù**: lỡ đúng phút thì tick kế tiếp vẫn bắt được mốc; và không gửi trùng vì sau khi gửi watermark vượt mốc. Các nhánh gate (tắt/chưa cấu hình/không người nhận/chưa tới giờ/đã gửi) trả về **không log** để tick mỗi phút không spam log. `send_digest` gửi **chỉ đơn `classification="genuine"`** được **đồng bộ kể từ lần gửi trước** (watermark, lọc theo Hub `created_at`); lần đầu (chưa có watermark) chỉ nhìn lại `MAIL_DIGEST_FIRST_RUN_LOOKBACK_HOURS` (24h). Mọi nhánh trong `send_digest` đều **đẩy watermark** để cửa sổ không phình. Cap `MAX_EMAIL_ORDERS` (200) mỗi email. `send_order_digest` (gửi ngay, bỏ qua lịch) vẫn còn cho admin/shell. Sửa lịch từ UI/Admin có hiệu lực ngay, **không cần redeploy**.
- **Gửi thủ công:** `POST /api/orders/send_email/` `{recipient, ids}` (action trên `OrderViewSet`) gửi **đúng các đơn đã chọn** (ids ∩ queryset đã lọc, tôn trọng filter/quyền) tới một địa chỉ bất kỳ — **đồng bộ** trong request (như `complete`/`cancel`): lỗi SMTP → 502, chưa cấu hình → 400, email sai → 400. Khác digest, gửi thủ công không lọc `genuine` (người dùng tự quyết).

**SMTP DB-driven:** kết nối dựng từ `MailSettings` (host/port/login/app-password) qua `mail.get_connection(backend=MAILER_EMAIL_BACKEND, ...)` — đổi tài khoản từ UI không cần redeploy; không dùng `EMAIL_*` của Django. App-password mã hóa Fernet (chung key WooCommerce `consumer_secret`), chỉ giải mã trong bộ nhớ lúc gửi, **không bao giờ** vào API/log. Test override `MAILER_EMAIL_BACKEND` sang locmem để assert trên `mail.outbox`.

## 6. Lớp tích hợp (`WooClient`)

Mọi giao tiếp WooCommerce tập trung tại đây để cô lập đặc thù của shared host:

- **Connection pool dùng chung** (`_POOL = httpx.Client(...)` cấp module, `HTTP_POOL_MAX_CONNECTIONS`/`_MAX_KEEPALIVE`/`_KEEPALIVE_EXPIRY`): mọi call đi qua một client keep-alive, **tái dùng TCP+TLS** qua từng trang/chunk/lần chạy thay vì bắt tay lại mỗi request (chi phí ẩn lớn nhất trên shared host latency cao). `httpx.Client` thread-safe nên fan-out (ThreadPoolExecutor mỗi batch task) dùng chung pool; auth/params/timeout vẫn truyền **per-call** nên fallback-401 không đổi. `follow_redirects=False`. (Pool KHÔNG tăng số request đồng thời — throughput vẫn do `*_BATCH_SIZE`/throttle quản.) Import ảnh dùng pool riêng `_IMG_POOL` (follow redirects) ở `apps/catalog/services`.
...
- **Batch sản phẩm**: `batch_products(create, update, delete)` (`POST /products/batch`) — đã hiện thực, cùng pattern auth/fallback-401, timeout 60s (ghi nặng hơn đọc). Trả `{create, update, delete}` mỗi item kèm `id`+`sku` (+ `images` đã sideload, mỗi ảnh kèm `id`+`src` site). **Caller (`push_products_to_site`) chịu trách nhiệm chia chunk ≤100 item.**
- **Đọc 1 sản phẩm**: `get_product(woo_id)` (`GET /products/{id}`) — cùng pattern auth/fallback-401/timeout; dùng cho sideload ảnh mô tả để biết id ảnh gallery hiện tại của một sản phẩm đã đồng bộ sẵn (không nằm trong batch lần này).
...
- **Không có batch endpoint** → một `batch_products` = nhiều request tuần tự, tự pacing
  ...
  Seam HTTP `_send` đi qua **connection pool dùng chung** `_POOL` (giống WooClient, cùng
  `HTTP_POOL_*`) nên các request tuần tự tái dùng keep-alive tới host canonical — đòn bẩy
  tốc độ chính khi Sapo không có batch; redirect thủ công + re-apply Basic auth giữ nguyên.
...
- **Celery worker** thực thi task; **Celery Beat** lên lịch (`poll_all_orders`, `check_all_sites`, và `dispatch_due_digests` — tick **mỗi phút** rồi tự xét mốc giờ trong `MailSettings.digest_times` để gửi email báo cáo đơn hàng, theo `CELERY_TIMEZONE`; lịch cấu hình từ UI thay cho `crontab` cứng). Broker = **Redis**. Push sản phẩm (`push_all_products` → `push_products_batch_task`) chạy on-demand (UI/Admin), không lên lịch.
- **Hai queue, hai worker riêng** (routing qua `CELERY_TASK_ROUTES` trong settings, theo tên task — task con batch đi theo queue của task cha):
  - `interactive` — task do người dùng bấm: `push_all_products`/`push_products_batch_task`, `pull_all_categories`/`pull_categories_batch_task`, `import_products_task` (route `import_*`). Worker riêng (`celery_worker_interactive` trong docker-compose) nên không bao giờ xếp sau job nền.

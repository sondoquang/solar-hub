...
- **ƯU TIÊN COMPONENT ANTD CÓ SẴN — BẮT BUỘC (đọc trước khi viết bất kỳ control nào).** Trước khi tự dựng một button/input/control mới bằng thẻ HTML thuần + Tailwind, **phải** kiểm tra antd đã có component tương ứng chưa và dùng nó. Không hand-roll lại thứ antd đã cung cấp.
  ...
  - Brand token đã map trong `src/lib/antdTheme.js` (`colorPrimary` = **violet `#978df8`** theo theme tối EvonHub, Button `primaryColor` = ink tối) → `<Button type="primary">` ra **đúng** màu thương hiệu, **không** cần tự đặt `bg-brand text-ink`.
...
- **Hướng thẩm mỹ hiện hành = theme tối EvonHub** (chi tiết token + convention màu ở `docs/frontend/ARCHITECTURE.md §9c-bis`): nền đen, accent violet `#978df8`, font **Manrope**. **Bắt buộc dùng semantic token** (`bg-surface-raised`, `text-muted`, `bg-brand/15`, `border-border`…), **không** rải hex hay utility màu sáng (`bg-white`, `text-slate-600`, `bg-amber-50`) trong component.
...
- Một cặp font có chủ đích (hiện dùng **Manrope** cho cả display lẫn body, đọc tốt cho bảng số liệu). **Tránh aesthetic "AI slop"**: không mặc định Inter/Arial/system font, không layout rập khuôn.

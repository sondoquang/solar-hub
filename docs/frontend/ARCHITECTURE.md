...
- **Orders / Sapo (`/sapo-unpaid-orders`)** — **toàn bộ** đơn Sapo (chỉ ghim `platform=sapo`; tên route giữ nguyên dù không còn giới hạn "chưa thanh toán"): bảng read-only với **2 badge** — trạng thái đơn (`OrderStatusBadge`) và trạng thái thanh toán (`PaymentStatusBadge`). Lọc theo **trạng thái đơn** (Đang xử lý/Hoàn thành/Đã hủy → `status`), **trạng thái thanh toán** (Chưa/Đã → `payment_status`, nhóm `unpaid`), website Sapo, khoảng ngày + tìm kiếm. Nút **Đồng bộ ngay** gọi `poll_now {platform: "sapo", status}` — đồng bộ đúng trạng thái đang chọn (mặc định processing/open; backend tự scope toàn bộ site Sapo, dedup theo store + gate `SAPO_ORDER_POLL_ENABLED`, kéo cả đơn đã & chưa thanh toán). 2 action mỗi dòng chỉ hiện khi áp dụng được — **Đánh dấu đã thanh toán** (`mark_paid`, ẩn khi đã thanh toán/đã hủy) và **Hủy đơn** (`cancel`, chỉ khi đơn còn mở) — đẩy thay đổi lên Sapo rồi re-sync. WooCommerce không bao giờ xuất hiện ở đây.
...
- **Cài đặt hệ thống → Mail SMTP (`/settings/mail`)** — form cấu hình tài khoản SMTP (host/port/TLS-SSL, email gửi + **mật khẩu ứng dụng**, tên/email hiển thị, danh sách email nhận báo cáo, bật/tắt digest 9h-16h) + nút **Gửi email thử**. Mật khẩu là write-only (không bao giờ tải về; chỉ nhập khi đổi). Email báo cáo tự động chỉ gồm đơn của **khách hàng thật** (`genuine`). Xem §9b.
...
**Orders (`/orders`)**: `src/api/orders.js` (hooks `useOrders/useOrderStats/usePollOrders/useCompleteOrder/useCancelOrder/useForwardOrdersBulk/useSendOrdersEmail` + hàm `exportOrdersPdf` tải PDF blob; list endpoint trả đủ `line_items` + thông tin khách trong mỗi row nên modal chi tiết **không fetch thêm**). Trang dùng `DataTable` (phân trang/sort server-side, filter bar: ngày + trạng thái + marketing + **phân loại** + gộp website/hosting + search debounce). Xem chi tiết theo kiểu **carousel dùng chung một modal** `OrderDetailModal` (nhận **mảng** đơn):
- Thân đơn tách ra component presentational `OrderDetailContent` (Descriptions + bảng line items), dùng cho cả xem 1 đơn lẫn carousel (Woo) lẫn modal đơn Sapo; dòng **"Phân loại"** hiển thị `ClassificationBadge` + điểm rủi ro + danh sách lý do (`risk_reasons_display`); dòng **"Thanh toán"** (`PaymentStatusBadge`) chỉ hiện khi đơn có `payment_status` — tức đơn Sapo, đơn Woo để trống nên không hiện.
...
- **Gửi mail đơn hàng thủ công**: tick chọn nhiều đơn → nút **"Gửi mail (N)"** ở `toolbarExtra` mở `SendOrdersEmailModal` (nhập một địa chỉ email người nhận) → `useSendOrdersEmail` (POST `/orders/send_email/` `{recipient, ids}`). Backend dựng email body HTML (giống chi tiết đơn) + PDF đính kèm (mỗi trang một đơn) từ tài khoản SMTP đã cấu hình ở **Cài đặt → Mail SMTP**, gửi đồng bộ và trả `{sent, recipient}`; toast kết quả + bỏ chọn. Nút disable cho tới khi email hợp lệ; lỗi cấu hình/SMTP hiện `detail` từ server.

**Đơn Sapo chưa thanh toán (`/sapo-unpaid-orders`)**: trang riêng cho nghiệp vụ Sapo (chỉ quan tâm **trạng thái thanh toán**). Dùng chung `src/api/orders.js` + query key `["orders"]`, ghim filter `platform=sapo` + `payment_status=unpaid` (đơn Woo không bao giờ lọt vào). `DataTable` rút gọn (bỏ phân loại/marketing/PDF), filter bar: ngày + gộp website Sapo (`useSites({platform:"sapo"})`) + search debounce; cột **"Trạng thái thanh toán"** dùng `PaymentStatusBadge` (mirror `OrderStatusBadge`). Mỗi dòng có **Xem chi tiết** (luôn hiện) + **2 thao tác** bọc `Popconfirm`: **Đánh dấu đã thanh toán** (`useMarkOrderPaid` → POST `/orders/{id}/mark_paid/`, ẩn khi đã paid/đã hủy) và **Hủy đơn** (`useCancelOrder`, nút danger); cả hai invalidate `["orders"]` nên đơn vừa thanh toán/hủy rời danh sách. **Xem chi tiết** mở `SapoOrderDetailModal` (1 đơn — màn Sapo không tick chọn nhiều nên không có carousel như Woo): thân dùng chung `OrderDetailContent`, footer lặp lại đúng 2 thao tác paid/cancel để thao tác ngay sau khi xem (thành công thì đóng modal). "Đồng bộ ngay" gọi `usePollOrders` scope các site Sapo (backend tự layer `financial_status=unpaid`). ⚠️ Đơn thanh toán ngoài luồng (trực tiếp trên Sapo) không được kéo về cập nhật — chỉ thao tác qua Hub mới chắc chắn đồng bộ.
...
**Khung app (`AppLayout`)**: layout dạng **sidebar trái** (logo Solar Hub, nav có icon, thẻ hỗ trợ ở đáy) + **topbar** (chuông thông báo + hồ sơ người dùng). Mục nav có `children` render qua component dùng chung **`NavGroup`** (mở rộng/thu gọn, tự mở khi route hiện tại thuộc nhóm; sidebar collapsed thì chỉ hiện icon link tới route chính của nhóm): **Sản phẩm** gồm Danh sách sản phẩm (`/products`) / Danh mục (`/categories`); **Website** gồm Hosting / Quản lý website / Lịch sử kiểm tra; **Cài đặt hệ thống** gồm Mail SMTP (`/settings/mail`). Các route đã có (`/`, `/orders`, `/sapo-unpaid-orders`, `/products`, `/categories`, `/sites`, `/hostings`, `/health-checks`, `/reports`, `/settings/mail`, `/login`) dùng `NavLink` (mục "Đơn Sapo chưa TT" là `NavRow` phẳng cạnh "Đơn hàng"); `/settings` redirect sang `/settings/mail`.
...
- `<ConfigProvider theme={...} locale={viVN}>` — theme antd nằm ở `src/lib/antdTheme.js`, **ánh xạ đúng design token** trong `src/index.css`. Đổi token thì sửa cả hai nơi.
...
### 9c-bis. Design system — EvonHub dark theme (hiện hành)

Dashboard dùng **một theme tối duy nhất** theo bảng thiết kế **EvonHub** (chưa có toggle sáng/tối). Quy tắc: **dùng semantic token, không rải hex/utility màu sáng** (`bg-white`, `text-slate-600`, `bg-amber-50`…) trong component.

- **Nền tảng token** ở `src/index.css` `:root` (nguồn raw value duy nhất, Tailwind + antd cùng ánh xạ):
  - Accent thương hiệu: **violet `#978df8`** (`--color-brand`). Lưu **2 dạng**: hex `--color-brand` (cho antd/NProgress/inline style) và kênh RGB `--color-brand-rgb: 151 141 248` (để Tailwind áp opacity được — `bg-brand/15`, `ring-brand/40`).
  - Bề mặt: `--color-surface` `#000000` (nền trang) · `--color-surface-raised` `#1a1b1e` (card/panel) · `--color-surface-muted` `#1f2026` (inset, dropdown, modal).
  - Chữ: `--color-ink` `#f8fafc` (mạnh — tiêu đề/giá trị) · `--color-text` `#cbd5e1` (body) · `--color-muted` `#94a3b8` (phụ).
  - Viền: `--color-border` `#2a2c33` · `--color-border-strong` `#3a3d46`. Status (`success/warning/danger`) chỉnh sáng cho nền tối (`#22c55e`/`#f59e0b`/`#ef4444`). Radius `--radius` 6px / `--radius-lg` 10px.
- **Font:** **Manrope** (Google Fonts, nạp ở `index.html`) cho cả display lẫn body — thay Be Vietnam Pro cũ.
- **antd** (`src/lib/antdTheme.js`): `algorithm: darkAlgorithm`; `colorPrimary` violet; `colorBgBase #000`, `colorBgContainer #1a1b1e`, `colorBgElevated #1f2026`; Button `primaryColor` = ink tối (`#16171a`) vì chữ trắng trên violet không đạt contrast AA; override Input/Select/Table/Switch sang accent violet. Nhờ darkAlgorithm, **mọi component antd (Table/Modal/Form/Select/Tabs/Pagination…) tự tối** — chỉ phần hand-rolled (AppLayout, Dashboard card, badge) mới cần class token.
- **Tailwind token** (`tailwind.config.js`): `brand` (dạng `rgb(var(--color-brand-rgb)/<alpha-value>)`), `brand-hover`, `surface`/`surface-raised`/`surface-muted`, `ink`/`text`/`muted`, `border`/`border-strong`, status. Font `display`/`sans` = Manrope.
- **Convention màu cho component hand-rolled:** card = `bg-surface-raised border border-border`; chip/badge trạng thái = `bg-<color>-500/15 text-<color>-300|400` (vd `bg-green-500/15 text-success`, `bg-blue-500/15 text-blue-300`); chip trung tính = `bg-white/10 text-slate-300`; hover nền = `hover:bg-white/5`; nav/active = `bg-brand/15 text-brand`; tách dòng = `border-border`/`divide-border`.
...
- Phân quyền nhiều vai trò chi tiết, đa ngôn ngữ — chưa làm. Dashboard hiện là **theme tối EvonHub cố định** (xem §9c-bis); **toggle sáng/tối** chưa làm.

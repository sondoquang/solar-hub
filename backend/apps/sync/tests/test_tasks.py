...
def test_poll_all_orders_chunks_into_batches(monkeypatch):
    ...
    dispatched = []
    monkeypatch.setattr(
        tasks.poll_sites_batch_task,
        "delay",
        lambda chunk, status, date_from, date_to: dispatched.append((chunk, status)),
    )

    result = tasks.poll_all_orders()
    assert result == {"status": "processing", "sites": 5, "batches": 3}
...
def test_poll_all_orders_filters_to_selected_sites(monkeypatch):
    ...
    assert result == {"status": "completed", "sites": 1, "batches": 1}
...
def test_poll_all_orders_passes_date_range_to_batch(monkeypatch):
    ...
    monkeypatch.setattr(
        tasks.poll_sites_batch_task,
        "delay",
        lambda chunk, status, date_from, date_to: dispatched.append(
...
def test_poll_sites_batch_task_polls_each_site(monkeypatch):
    ...
    def _fake_poll(site, status, date_from=None, date_to=None):

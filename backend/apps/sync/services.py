...
PRODUCT_OPERATION = "push_products"


def _runs_queryset(operation, params=None):
    """Grouped queryset of runs (newest first), one row per ``run_id``, for one
    ``operation`` (``pull_categories`` or ``push_products`` — both fan out one
    SyncLog row per site, so the roll-up is identical).

    Legacy rows written before run_id existed are excluded — they carry no
    snapshot, so there is nothing to report. ``params`` (the request query
    params) optionally filter the list:
    ...
    logs = SyncLog.objects.filter(
        operation=operation, run_id__isnull=False, is_deleted=False
    )
    ...
    if site and str(site).isdigit():
        run_ids_for_site = SyncLog.objects.filter(
            operation=operation, site_id=int(site), is_deleted=False
        ).values("run_id")
    ...
    if search:
        ...
        try:
            ...
            pass
        run_ids_matched = (
            SyncLog.objects.filter(operation=operation, is_deleted=False)
            .filter(match)
            .values("run_id")
        )
...
def category_runs_queryset(params=None):
    """Grouped category-pull runs — see ``_runs_queryset``."""
    return _runs_queryset(CATEGORY_OPERATION, params)


def product_runs_queryset(params=None):
    """Grouped product-push runs — see ``_runs_queryset``."""
    return _runs_queryset(PRODUCT_OPERATION, params)
...
# Per-item reject codes that mean "the product already exists / SKU clash" rather
# than a genuine error — the UI tints these differently (a name match landing on
# a product whose SKU collides is expected friction, not a broken push).
_DUPLICATE_FAILURE_CODES = {
    "product_invalid_sku",
    "woocommerce_product_invalid_sku",
    "duplicate_sku",
    "term_exists",
}
...
def _classify_failure(code: str) -> str:
    """``"duplicate"`` for an already-exists/SKU-clash reject, else ``"error"``."""
    return "duplicate" if code in _DUPLICATE_FAILURE_CODES else "error"


def _product_summary(run_id, started_at, logs) -> dict:
    """Roll a product-push run's per-site rows up into one report row.

    Mirrors ``_summary`` (shared duration/triggered_by/site_label logic) but the
    counters are push-shaped: created/updated/deleted land on the SyncLog
    columns, while adopted (name-matched) and failed live in ``detail``."""
    finished = max((log.created_at for log in logs), default=started_at)
    starts = [log.started_at for log in logs if log.started_at] or [started_at]
    duration = (finished - min(starts)).total_seconds() if logs else 0
    ...
    return {
        ...
        "started_at": started_at,
        "site_count": len(logs),
        "total_created": sum(log.created_count for log in logs),
        "total_updated": sum(log.updated_count for log in logs),
        "total_deleted": sum(log.deleted_count for log in logs),
        "total_adopted": sum(
            int((log.detail or {}).get("adopted_count") or 0) for log in logs
        ),
        "total_failed": sum(len((log.detail or {}).get("failed") or []) for log in logs),
        "error_count": sum(1 for log in logs if log.status == SyncLog.Status.ERROR),
        "status": _rollup_status(logs),
        "duration_seconds": max(0, round(duration)),
        "triggered_by": triggered_by,
...
def summarize_product_runs(run_rows) -> list[dict]:
    """Roll the page's product runs up in Python (mirror ``summarize_runs``)."""
    run_ids = [row["run_id"] for row in run_rows]
    logs_by_run: dict = {}
    for log in SyncLog.objects.filter(
        operation=PRODUCT_OPERATION, run_id__in=run_ids, is_deleted=False
    ).select_related("site", "triggered_by"):
        logs_by_run.setdefault(log.run_id, []).append(log)
    return [
        _product_summary(row["run_id"], row["started_at"], logs_by_run.get(row["run_id"], []))
        for row in run_rows
    ]


def product_run_stats(params=None) -> dict:
    """Stat-card counts for the product Lịch sử đồng bộ tab (mirror
    ``category_run_stats``): total runs + success/partial/error split over a
    window (default last 30 days), plus the most-recent run as ``last_run``."""
    params = dict(params or {})
    if not params.get("date_from") and not params.get("date_to"):
        params["date_from"] = (timezone.localdate() - timedelta(days=30)).isoformat()

    runs = list(
        product_runs_queryset(
            {
                "site": params.get("site"),
                "date_from": params.get("date_from"),
                "date_to": params.get("date_to"),
    ...
    if runs:
        ...
        logs = list(
            SyncLog.objects.filter(
                operation=PRODUCT_OPERATION, run_id=top["run_id"], is_deleted=False
            ).select_related("site", "triggered_by")
        )
        summ = _product_summary(top["run_id"], top["started_at"], logs)
        last_run = {
            "run_id": summ["run_id"],
            "started_at": summ["started_at"],
            "site_count": summ["site_count"],
...
def _product_site_row(log) -> dict:
    """One per-site row of a product run, null-safe (``site`` is SET_NULL on
    delete → fall back to the ``detail`` snapshot). ``failed`` carries each
    rejected SKU + Woo's error code/message, tagged ``kind`` so the UI tints
    duplicate-SKU rejects apart from genuine errors."""
    site = log.site
    detail = log.detail or {}
    hosting = site.hosting if site else None
    failed = [
        {
            ...
            "kind": _classify_failure(f.get("code", "")),
        }
        for f in (detail.get("failed") or [])
    ]
    return {
        "site_id": site.id if site else None,
        "site_name": site.name if site else detail.get("site_name", ""),
        ...
        "adopted_count": int(detail.get("adopted_count") or 0),
        "adopted": detail.get("adopted") or [],
        "ambiguous": detail.get("ambiguous") or [],
        "recreated_stale": int(detail.get("recreated_stale") or 0),
...
def product_run_detail(run_id) -> dict | None:
    """Summary + per-site rows (incl. failure reasons) of one product run, or None."""
    logs = list(
        SyncLog.objects.filter(
            operation=PRODUCT_OPERATION, run_id=run_id, is_deleted=False
        )
        .select_related("site__hosting", "triggered_by")
        .order_by("created_at")
    )
    if not logs:
        return None
    summary = _product_summary(run_id, min(log.created_at for log in logs), logs)
    summary["sites"] = [_product_site_row(log) for log in logs]
    return summary
